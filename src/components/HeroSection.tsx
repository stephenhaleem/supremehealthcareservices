...
const HeroSection = () => {
  ...
  <section className="animate-section border-b border-primary/10 bg-background">
    <div className="container grid min-h-[700px] items-stretch gap-0 py-0 lg:grid-cols-[1.02fr_.98fr]">
      <div className="flex flex-col justify-between bg-background px-6 py-16 md:px-12 lg:px-16 lg:py-20">
        <div className="animate-fade-in">
          ...
          <div className="mt-9 flex flex-wrap gap-3">
            <Button
              asChild
              size="lg"
              className="h-14 rounded-none px-7 text-xs font-bold uppercase tracking-[0.14em]"
            >
              <Link to="/contact">
        ...
        <div className="mt-14 grid gap-px border border-border bg-border sm:grid-cols-3">
          {["CPR certified", "Police checked", "Available 24/7"].map((item) => (
            <div
              key={item}
              className="motion-hover flex items-center gap-2 bg-background px-4 py-4 text-[11px] font-semibold uppercase tracking-[0.1em] text-foreground"
            >
      ...
      <div className="group relative min-h-[420px] overflow-hidden bg-secondary lg:min-h-0">
        ...
        />
        <div className="absolute inset-x-5 bottom-5 grid grid-cols-[1fr_auto] items-end gap-4 bg-background/95 p-6 backdrop-blur-sm md:inset-x-auto md:bottom-7 md:left-7 md:max-w-sm">
          <div>
